...
public class SolicitacaoDAOWalterPaulo {
	...
	public void incluir(SolicitacaoWalterPaulo solicitacao) 
		...
		ps.setString(1, solicitacao.getDescricao().trim());
	...
	public List<SolicitacaoWalterPaulo> listar() 
		...
		String sql = "SELECT s.id AS idSolicitacao,"
				...
				+ "LEFT JOIN status_retorno sr ON rs.id_status_retorno = sr.id ORDER BY s.id";
		PreparedStatement ps = conn.prepareStatement(sql);
		...
		while (rs.next()) {
			...
			solicitacao.setDescricao(rs.getString("descricaoSolicitacao").trim());
